...
class XbeeCommunication:
    ...
    def __init__(
        ...
        self.XbeesValvesSystem = {
            "Agent_1": "0013A20041573102",
    ...
    def data_receive_callback(self, xbee_message):
        ...
        if self.message[0] == "IRRIG":
            ...
            if self.message[1].split(";")[0] == "COMPLETE":
                ...
                self.save_data_Xbee(
                    f"{self.Path_Data}/Irrigation_finished.txt",
                    self.message[1].split("\x00")[0],
                )
                ...
                self.save_data_Xbee(
                    f"{self.Path_Data}/Irrigation_started.txt", self.message[1]
                )
                self.numberReceivedOrders += 1
                try:
                    self.FireBase.ResultIrrDoc_ref.update({"IrrigationState": "ON"})
                except:
            ...
            print("sensores:")
            self.messageSens = (
                self.message[1].split("\x00")[0].split("End")[0].split(",")
            )
            self.save_data_Xbee(
                f"{self.Path_Data}/Sensor_data.txt",
                self.message[1].split("\x00")[0].split("End")[0],
            )
            self.sensors.allSensors = [
                float(x) for x in self.messageSens[1 : len(self.messageSens) - 1]
            ]

            for x in range(0, 4):
                self.sensors.allSensors[x] = round(
                    self.calc_volumetricWaterContent(self.sensors.allSensors[x], 4.75),
                    2,
                )
            ...
            if self.sumSensors > 0:
                ...
                data = {
                    ...
                    "Longitude": float(5.5),
                    "Latitude": float(-78.4),
                    "SM_1": self.Allsensors[0],
                    "SM_2": float(self.Allsensors[1]),
                    "SM_3": float(self.Allsensors[2]),
                    "Env_Temp": float(self.Allsensors[4]),
                    "RH": float(self.Allsensors[5]),
                    "CO2": float(self.Allsensors[6]),
                    "Canopy_Temp": float(self.Allsensors[7]),
                    "CS_Temp": float(self.Allsensors[8]),
                    "Irrig_Pres_Rate": float(self.PrescIrrig),
                    "Irrig_Pres_Time": float(self.timeIrrig),
                    "Date_Time": dateHour,
                }
    ...
    def save_data_Xbee(self, directory, message):
        ...
        self.SaveFile.write(
            f"{str(datetime.now()).split()[0]},{str(datetime.now()).split()[1]},{message}\n"
        )
        self.SaveFile.close()
        ...
        try:
            ...
            self.remote_device = RemoteXBeeDevice(
                ...
                XBee64BitAddress.from_hex_string(
                    self.XbeesValvesSystem[f"Agent_{Agent}"]
                ),
            )
